...
export const createCard = function (
  ...
  if (cardId === currentUserId) {
    ...
    deleteButton.style.display = 'block'; // или deleteButton.classList.remove('hidden'); если у вас есть CSS класс для скрытия
  } else {
    // Если карточка не создана текущим пользователем, скрываем кнопку удаления
    deleteButton.style.display = 'none'; // или deleteButton.classList.add('hidden');
  }
