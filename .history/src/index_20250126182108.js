...
//   .catch((err) => { 
//     console.log(`Что-то пошло не так. Ошибка: ${err}`); //выведем сообщение ошибки в консоль
//   }); 
...
//   // если идёт загрузка
//   if (isLoading) { 
//     button.textContent = "Сохранение..."; //то меняет текст кнопки на сохранение
//     button.disabled = true; 
//   } else { 
//     button.textContent = "Сохранить"; //когда загрузка завершилась, то возвращает текст кнопки и отключает её
//     button.disabled = false; 
//   } 
// } 
 
// //функция обработки формы профиля, обновляет информацию профиля на странице
// function handleProfileFormSubmit(res) { 
//   profileInfo.dataset.userId = res._id; //здесь сохраняет id-пользователя 
//   profileImage.style.backgroundImage = `url(${res.avatar})`; //устанавливает изображение профиля
//   profileTitle.textContent = res.name; //обновляет текст с именем
//   profileDescription.textContent = res.about; //обновляет текст описания профиля
// } 
 
// //Функция обработки карточек, на вход массив карточк и id-владельца
// function handleInfoCards(res, ownerId) { 
//   res.forEach((card) => { //проходится по каждой карточке  
//     cardsContainer.append( //и добавляет в контейнер карточек
