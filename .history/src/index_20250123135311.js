...
fetch('https://nomoreparties.co/v1/wff-cohort-31/cards', {
  ...
  const changeAvatar = (avatarUrl) => {
     fetch('https://nomoreparties.co/v1/wff-cohort-31/users/me/avatar', {
      ...
      body: JSON.stringify({
        avatar: avatarUrl, // Передаем ссылку на новый аватар
      })
    })
    .then(res => {
      if (res.ok) {
        return res.json(); // Если запрос успешен, преобразуем ответ в JSON
      }
      return Promise.reject(`Ошибка: ${res.status}`); // Обрабатываем ошибку, если запрос не успешен
    });
  }
